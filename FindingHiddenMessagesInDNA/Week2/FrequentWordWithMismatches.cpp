...
void generateStrings(string &temp){
    if((int)temp.size() == k){
        vector<int> see1 = solve(d,text,temp);
        string co = temp;
        reverse(co.begin(),co.end());
        vector<int> see2 = solve(d,text,co);
        if((int)see1.size() + (int)see2.size() > coun){
            ans.clear();
            ans.push_back(temp);
            coun = (int)see1.size()+(int)see2.size();
        }
        else if((int)see1.size() + (int)see2.size() == coun)
            ans.push_back(temp);
        return;
    }
    for(int i = 0; i < 4;++i){
